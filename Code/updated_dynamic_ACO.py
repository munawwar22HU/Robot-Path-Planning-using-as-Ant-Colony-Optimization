...
class Dynamic_ACO:
    def __init__(self, ants, iterations, evaporation_factor, persistance, initial_pheramone, alpha, beta,  dim, obstacles, additional_obstacles) -> None:
        ...
        self.map = grid(self.dim, self.obstacles, self.additional_obstacles)
        self.grid = []
        self.initial_pheramone = initial_pheramone
        
    def set_grid(self, use_additional=False):
        ''' Create the nodes and edges in the grids '''
        self.grid = []
        obstacles = self.obstacles
        if use_additional:
            obstacles += additional_obstacles
        for y in range(self.dim):
            ...
            for x in range(self.dim):
                if (x,y) not in obstacles:
                    ...
                    row.append(Node((x,y), edges, self.initial_pheramone, self.alpha))
                else:
                    row.append(None)
            self.grid.append(row)
        
    def reset_pheramone(self):
        ''' Reinitialize the pheramone of each node  '''
        for (x, y) in self.additional_obstacles:
            self.grid[y][x] = None
        for y in range(self.dim):
            for x in range(self.dim):
                i_deleted = 0
                if self.grid[y][x] != None:
                    for i in range(len(self.grid[y][x].edges)):
                        if tuple(self.grid[y][x].edges[i-i_deleted]) in self.additional_obstacles:
                            self.grid[y][x].edges.pop(i-i_deleted)
                            self.grid[y][x].phermones = np.delete(self.grid[y][x].phermones, [i-i_deleted])
                            i_deleted += 1
                    self.grid[y][x].phermones = (self.grid[y][x].phermones + 10)/100
                    temp = self.grid[y][x].phermones**self.grid[y][x].alpha
                    self.grid[y][x].probability = temp/temp.sum()
    ...
    def generate_path(self, start_node, end_node):
        ...
        while path[-1] != tuple(end_node):
            ...
            # node = prev_node.edges[np.random.choice(range(len(prev_node.edges)), 1, prev_node.probability)[0]]
            path.append(node)
    ...
    def visualize(self, path, use_additional=False):
        ''' Visualize the given path over the grid '''
        self.map.visualize(path, use_additional)
...
aco = Dynamic_ACO(ants, iterations, evaporation_factor, persistance, initial_pheramone, alpha, beta, dim, obstacles30, additional_obstacles)
aco.set_grid(False)
path = aco.run((0,0), (dim-1,dim-1))
aco.visualize(path)
aco.visualize(path, True)
aco.reset_pheramone()
updated_path = aco.run((0,0), (dim-1,dim-1))
print('Initial Path:', path)
print('Updated Path:', updated_path)
aco.visualize(updated_path, True)