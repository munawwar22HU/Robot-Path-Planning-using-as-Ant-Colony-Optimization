...
dim = 30
obstacles = np.array(list(set(np.random.randint(1,dim*dim-1, 50))))
obstacles = list(np.array([obstacles%dim, obstacles//dim]).transpose())
...
while len(obstacles) < 300:
    item = list(obstacles[np.random.randint(0,len(obstacles))])
    d = directions[np.random.randint(0,4)]
    item[0] += d[0]
    item[1] += d[1]
    item = tuple(item)
    if item[0] >= 0 and item[0] < dim and item[1] >= 0 and item[1] < dim and item != (0,0) and item != (dim-1, dim-1) and item not in obstacles:
        obstacles.append(tuple(item))
